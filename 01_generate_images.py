#!/usr/bin/python3


from config import seed, dataset_dir, image_size, objects_types, train_size, validation_size, test_size

import random
import numpy as np
from tqdm import tqdm  # Import tqdm for progress tracking


def create_dirs(output_dir):
    from pathlib import Path
    import shutil

    # Define the root directory
    root_dir = Path(output_dir)

    # Define the subdirectories to be created
    subdirs = [
        'images/train',
        'images/validation',
        'images/test',
        #'argmax/train',
        #'argmax/validation',
        #'argmax/test'
    ]

    for subdir in subdirs:
        aux_path = root_dir / subdir
        if aux_path.exists():
            shutil.rmtree(aux_path)

    # Create each directory
    for subdir in subdirs:
        for type_ in objects_types:
            # Construct the full path
            path = root_dir / subdir / type_
            # Create the directory, including any necessary parent directories
            path.mkdir(parents=True, exist_ok=True)

    path = Path('./models/')
    path.mkdir(parents=True, exist_ok=True)

    print(f"Folder structure created under '{root_dir}'")




# Function to create a blank image and draw random objects
def create_random_object_image(image_size=(32, 32), seed=None):


    min_size = np.min(image_size)//20
    max_size = np.min(image_size)//4

    # Randomly select an object to draw
    object_type = random.choice(objects_types)

    # Randomize position, size, and thickness
    size_x = random.randint(min_size, max_size)
    size_y = size_x
    center_x = random.randint(size_x//2+2, image_size[1] - size_x//2-2)
    center_y = random.randint(size_y//2+2, image_size[0] - size_y//2-2)
    thickness = random.randint(1, max(1, min(size_x//4, size_y//4)))

    #print(f'center={center_x, center_y}, size={size}, thickness={thickness}, type={object_type}')

    image = create_object_image(image_size, (center_x, center_y), (size_x, size_y), thickness, object_type)

    return image, object_type, size_x, size_y, center_x, center_y, thickness




def create_object_image(image_size, object_center, object_size, object_thickness, object_type):

    import cv2
    import numpy as np

    img_size = list(image_size) + [3]

    thickness = object_thickness
    size_x, size_y = object_size
    center_x = object_center[0]
    center_y = object_center[1]


    # Create a blank black image
    image = np.zeros(img_size, dtype=np.uint8)

    color = (255, 255, 255)  # White color for the objects

    if object_type == 'cross':
        # Draw a cross
        cv2.line(image, (center_x - size_x, center_y), (center_x + size_x, center_y), color, thickness)
        cv2.line(image, (center_x, center_y - size_y), (center_x, center_y + size_y), color, thickness)

    elif object_type == 'circle':
        # Draw a circle
        cv2.circle(image, (center_x, center_y), size_x, color, thickness)

    elif object_type == 'square':
        # Calculate the top-left and bottom-right points for the square
        size_y = size_x
        top_left = (center_x - size_x, center_y - size_y)
        bottom_right = (center_x + size_x, center_y + size_y)
        cv2.rectangle(image, top_left, bottom_right, color, thickness)

    elif object_type == 'rectangle':
        # Calculate the top-left and bottom-right points for the rectangle
        top_left = (center_x - size_x, center_y - size_y)
        bottom_right = (center_x + size_x, center_y + size_y)
        cv2.rectangle(image, top_left, bottom_right, color, thickness)

    # Return the generated image
    return image





# Función para generar n imágenes
def generate_images(output_dir, image_size, train_size, validation_size, test_size, seed=None):

    from pathlib import Path
    import cv2
    import pickle

    # Generate directory structure
    create_dirs(output_dir)

    # Definir los tamaños y carpetas de salida
    sizes = {'train': train_size, 'validation': validation_size, 'test': test_size}
    infos = list()

    # Crear carpetas si no existen
    for subset, size in tqdm(sizes.items(), desc="Processing Subsets", unit="subset"):
        subset_dir = Path(output_dir + '/images') / subset
        subset_dir.mkdir(parents=True, exist_ok=True)

        # Generar y guardar las imágenes
        for i in tqdm(range(size), desc="Processing Images", unit="image"):
            image_params = create_random_object_image(image_size, seed)
            image = image_params[0]
            #keys = ['type', 'size', 'center', 'thickness']
            #params = [
            #        image_params[1],
            #        image_params[2],
            #        image_params[3:5],
            #        image_params[5],
            #        ]
            keys = ['type', 'size_x', 'size_y', 'center_x', 'center_y', 'thickness']
            params = image_params[1:]  # Extract all but the image
            info = dict(zip(keys, params))
            info['set'] = subset
            info['index'] = i
            #info['params'] = params
            filename = subset_dir / f"{params[0]}/{subset}_{i:010d}.png"
            cv2.imwrite(str(filename), image)
            infos.append(info)

    # Save the variable to a file
    with open(output_dir + '/dataset_info.pkl', 'wb') as file:  # 'wb' means write in binary mode
        pickle.dump(infos, file)

    print(f"Imágenes generadas y guardadas en '{output_dir}'")

    return infos






if __name__ == "__main__":

    random.seed(seed)
    ## Generate and display the image
    #random_image = create_random_object_image(image_size=(image_size,image_size))[0]
    #import cv2
    #cv2.imshow('Random Object', random_image)
    #cv2.waitKey(0)
    #cv2.destroyAllWindows()

    # Llamada a la función para generar las imágenes
    infos = generate_images(dataset_dir,
                            image_size=(image_size,image_size),
                            train_size=train_size,
                            validation_size=validation_size,
                            test_size=test_size,
                            seed=seed,)

    for info in infos[:5]:
        print(info)





















